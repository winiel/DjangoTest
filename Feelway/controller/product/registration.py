...
def index(request):
    ...
    test_img_path = base_dir + "/Feelway/lib/utils/test_img";
    ...
    driver_path = base_dir + "/Feelway/lib/utils/chromedriver"
    ...
    ##  브랜드
    Select(driver.find_element_by_css_selector("form[name=up_form] select[name=brand_no]")).select_by_visible_text(
        "PRADA")
    ...
    ##  상품 종류
    ### 대분류
    Select(driver.find_element_by_css_selector("form[name=up_form] select[name=cate_no]")).select_by_visible_text(
        "가방/핸드백");

    ### 소분류
    Select(driver.find_element_by_css_selector("form[name=up_form] select[name=sub_cate_no]")).select_by_visible_text(
        "크로스백");
    ...
    ####  사진등록하기 - window (s)
    img_1 = test_img_path + "/prada_1.jpg";
    img_2 = test_img_path + "/prada_2.jpg";
    ...
    ### 배송 - 배송방법
    Select(
        driver.find_element_by_css_selector("form[name=up_form] select[name=sending_method]")).select_by_visible_text(
        "택배");
    ...
    driver.find_element_by_xpath(
        "//input[@type='image'][@src='http://icon.feelway.com/feel_image_02/btn29.gif']").click();
    ...
    return HttpResponse("<h1>End Reg Product</h1>")